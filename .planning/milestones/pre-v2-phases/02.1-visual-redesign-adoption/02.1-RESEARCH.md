...
## Resolved Browser Smoke Strategy

**Decision:** Use manual/tool-assisted browser smoke in installed Chrome with no new browser test package. [VERIFIED: SPEC.md] [VERIFIED: package.json]

- Start the app with `just dev`; use manual Chrome DevTools viewports or available browser-control tooling to inspect the running app.
- Do not add Playwright, Vitest browser mode, or any new browser automation dependency for this phase.
- Capture findings in the Plan 04 summary alongside the automated `just ci` result.

**Viewport checklist:**

- Desktop: 1440x900 browser viewport.
  - Check closed TopBar C2, open setup popover, desktop pad composition, all variation picker families, active orange only on sounding/latched pads, and no progression/sequencer/queued-toast surfaces.
- iPad-sized: 1024x768 browser viewport.
  - Check stacked TopBar/setup reachability, 44px custom targets where practical, all setup fields reachable, variation controls readable, and both pad rows playable.
- iPhone landscape: 844x390 browser viewport.
  - Check the short-viewport scroll escape, no incoherent overlap, no clipped primary labels, setup popover dismissibility, and reachable pad rows.
